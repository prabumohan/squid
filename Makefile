...
create:
	docker run --privileged --name $(CONTAINER_NAME) --restart=always  -d -v /sys/fs/cgroup:/sys/fs/cgroup:ro -p 8080:3128  -v $(MAKE_DIR)/hosts_$(ENV):/etc/hosts $(IMAGE_NAME):$(IMAGE_TAG)
