...
create:
	docker run --privileged --name $(CONTAINER_NAME) --restart=always  -d -v /sys/fs/cgroup:/sys/fs/cgroup:ro -p 8080:8080 -v $(MAKE_DIR)/squid_$(ENV).conf:/etc/squid/squid.conf -v $(MAKE_DIR)/hosts_$(ENV):/etc/hosts $(IMAGE_NAME):$(IMAGE_TAG)
